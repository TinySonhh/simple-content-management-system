...
appName = "app"
# continue or fresh
buildOptions = "continue"

if len(sys.argv) > 1:
    path = sys.argv[1]    
    appName = os.path.basename(os.path.normpath(path))
    if len(sys.argv) > 2:
        buildOptions = sys.argv[2]
        if buildOptions != "fresh" and buildOptions != "continue":
            buildOptions = "continue"
else:
    path = "."
    buildOptions = "continue"
...
print("Create the _revision\latest.php file if not existed")
...
revisionHistoryFile.write("<?php return \"" + current_time_rev + "\"; ?>")
...
if os.path.exists(lastChangedFileName) :    
    if buildOptions == "fresh":
        lastChangedDateTime = "1900-01-01 00:00:00"
    else:
...
extensions = [
    ".js", ".php", ".html" , ".css", ".ts",
    ".png", ".jpg", ".jpeg", ".ico", ".gif", ".svg", ".webp", ".tif", ".tiff",
    ".mp3", ".wav", ".ogg", ".aiff", ".flac", ".aac",
    ".json",
...
for root, dirs, filenames in os.walk(path):
    for filename in filenames:
        if "google-api" not in root and ( (os.path.splitext(filename)[1] in extensions) or (filename in extensions) ) and datetime.fromtimestamp(os.path.getmtime(os.path.join(root, filename))) > time_after:
